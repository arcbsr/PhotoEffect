        ...
        fun attemptLoadImage() {
            ImageLoader.getInstance().loadBitmap(context,
                ...
                object : OnImageLoadedListener2 {
                    ...
                    override fun onErrorLoaded(url: String?, position: Int) {
                        if (retryCount < MonsterApiClient.MAX_RETRIES) {
                            retryCount++
                            RLog.e("Retrying image load attempt $retryCount for URL: $url")
                            Handler(Looper.getMainLooper()).postDelayed({
                                attemptLoadImage()
                            }, MonsterApiClient.RETRY_DELAY)
                        } else {
                            callback.onError(Exception("Failed to load image after ${MonsterApiClient.MAX_RETRIES} attempts"))
                        }
                    }
                })
        }
